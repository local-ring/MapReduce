...
In the end, shuffler will send `END_OF_DATA` to all reducers to tell them that it has finished sending data. The reducers will receive the data and start processing the data. It will store all the intermediate data in a temporary file. The temporary file will be named as `data_reducer_{reducer_id}_{task_type}.txt`. For example, `data_reducer_0_wordcount.txt` is the temporary file for reducer 0 in the wordcount task.

After get the `END_OF_DATA` message from shuffler, the reducer will send the data in the temporary file to the master node through the `PUSH` socket. The master node will receive the data and merge all the data from reducers. The master node will write the final result to the output file.

![communication](./img/dataflow.jpg)

**Warning:**
 
 The data stucture I use for transfering key-value pair is `json`, or in the format `key value` where `key` should be a word in both case and `value` can be `"1"` or `(docname, "1")` (we need to convert all to string or `json` for network communication). So we need no space in file name for inverted index job becasue we use space as delimiter for key-value pair during transfering data between master and workers.

 I uses multithreading to concurrently send data to different mappers. This is beneficial for performance, as it allows multiple mappers to start processing their respective data chunks without having to wait for others to begin. There might be the overhead of context switching between threads. Also, ZeroMQ is not thread-safe. But durng the testing, I didn't encounter any problem.


## Test Cases

- `t1`: a simple test case with 4 mapper and 2 reducer for wordcount task, the file contains 1 a, 2 b, 3 c, 4 d and so on, until 26 z. We can see the output is correct. Note that the second mapper does not have any data to process, which can be seen from the temporary file `data_mapper_1_wordcount.txt`. On the other side, the reducer have relatively balanced workload, which can be seen from the temporary file `data_reducer_0_wordcount.txt` and `data_reducer_1_wordcount.txt`: first reducer have 159 key value pairs and second reducer have 192 key value pairs.

- `t2`: with 5 mapper and 3 reducer for wordcount task, and the file need to be processed is way larger than the previous one one of sherlock holmes book downloaded from Project Gutenberg. We can see the mapreduce program can handle large file and it can get final result without any problem. The unbalanced workload issue disappears in this case. Since we don't know the correct answer like the first one, so we can't compare the output with the expected output. But we noticed some weird thing, very small count of word "the". This may related to my parser of text, which means I don't preprocess the text well. Need to investigate further.

- `t3`: with 2 mapper and 2 reducer for inverted index task, with three doc file consisting of some small letter combinations, so that the expected output is easy to check. We can see the output is correct. Through the temp files, the workload is balanced.

- `t4`: with 8 mapper and 5 reducer for inverted index task, with 14 files in total generated by the script I wrote `generate.py`. Each file contains some random number of letters and being shuffled. The expected output (letter distribution) is also generated by the script. After comparing the output with the expected output, we can see the output is correct. The workload is balanced.

## Future Work

- Use RPC for passing map and reduce functions to workers.
- Use Https server for master node to receive instructions from users.
- Use multiple sockets for the shuffle process communicating with reducers and mappers.
- Investigate the weird thing in test case 2.
- Check Hadoop configuration and see if we can implement some of them in our program.